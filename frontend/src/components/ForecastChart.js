    ...
    <div>
      ...
      {forecast && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
          {[
            { label: 'Prophet',  value: forecast.prophet,  color: '#a78bfa' },
      ...
      {forecast && (
        <div style={{ background: '#1e293b', borderRadius: '12px', padding: '16px 20px', border: '1px solid #334155', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span style={{ color: '#64748b', fontSize: '14px' }}>Model Confidence:</span>
          <div style={{ flex: 1, background: '#0f172a', borderRadius: '99px', height: '8px' }}>
            <div style={{ width: `${forecast.confidence_pct}%`, background: forecast.confidence_pct > 80 ? '#22c55e' : '#f59e0b', height: '8px', borderRadius: '99px' }}></div>
          </div>
