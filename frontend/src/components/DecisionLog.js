        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#0f172a' }}>
              {['Product', 'Stock', 'Forecast', 'Supplier', 'Qty', 'Confidence', 'PO Ref', 'Status', 'Actions'].map(h => (
          ...
          <tbody>
            {decisions.length === 0 ? (
              <tr><td colSpan={9} style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>No decisions yet</td></tr>
            ) : decisions.map((d) => (
              <tr key={d.id} style={{ borderTop: '1px solid #334155' }}>
                ...
                <td style={{ padding: '12px 16px', color: '#64748b', fontSize: '11px' }}>{d.po_id}</td>
                <td style={{ padding: '12px 16px' }}>
                  <span style={{
